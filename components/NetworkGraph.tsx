...
const NetworkGraph: React.FC<NetworkGraphProps> = ({ history, error }) => {
  ...
  return (
    ...
    >
       ...
       {showDebug ? (
         <div className="flex-1 bg-black/40 rounded p-2 overflow-auto font-mono text-[10px] text-green-400 whitespace-pre border border-white/10 h-[200px]">
           <div>Status: {error ? `Error (${error})` : 'OK'}</div>
           {hasData ? JSON.stringify(history.slice(-3), null, 2) : "No data in history array."}
         </div>
       ) : (
         <div className="flex-1 w-full relative" style={{ height: '200px', minHeight: '200px' }}>
           {!hasData && (
